...
MDNSEOF
    ...
    # Unicast DNS via dnsmasq — reliable fallback for flaky mDNS
...
_setup_dnsmasq() {
    ...
    cat > "$conf" << EOF
# AGMind unicast DNS — reliable alternative to mDNS
# Auto-generated by install.sh — do not edit manually
#
# Clients should add ${server_ip} as DNS server for stable
# agmind-*.local resolution without multicast dependency.
...
# DNS only: no DHCP, no upstream forwarding
port=53
no-resolv
no-hosts

# Authoritative for .local zone — all other queries get REFUSED
local=/local/
