...
- ✅ **v2.8 New Services + Wizard Simplification** — Phases 31-33 (shipped 2026-03-30)
...
<details>
<summary>✅ v2.8 New Services + Wizard Simplification (Phases 31-33) — SHIPPED 2026-03-30</summary>
...
Plans:
- [x] 33-01-PLAN.md — Docker-compose service definitions + versions.env + SearXNG config
- [x] 33-02-PLAN.md — Wizard steps + compose profiles + health + credentials + secrets
...
## v2.9 Bugfixes & Hardening

### Phase 34: Bugfixes — image validation, retry logic, service mapping dedup

**Goal:** Исправить BUG-V3-030 (image validation timeout), BUG-V3-043 (exponential backoff для stuck containers), BUG-V3-044 (RELEASE tag fallback), параллелизировать image validation, вынести дублированные service mappings в shared lib.
...
1. `_check_image_exists()` использует таймаут 20с (или конфигурируемый через `IMAGE_VALIDATION_TIMEOUT`)
2. `_get_registry_token()` имеет retry с backoff (3 попытки, 5с между ними)
3. `_retry_stuck_containers()` использует exponential backoff (10s→20s→40s)
4. RELEASE tag записывается даже без файла RELEASE (fallback на git describe или versions.env hash)
5. `validate_images_exist()` запускает проверки параллельно (до 5 concurrent)
6. Service mappings определены в одном месте (`lib/service-map.sh`), импортируются в `lib/health.sh` и `scripts/update.sh`
7. Все изменения проходят `shellcheck` без ошибок
8. Существующие профили (LAN/VPN/VPS) не сломаны

**Plans:** 2/2 plans complete

Plans:
- [ ] 34-01-PLAN.md — Image validation timeout+retry, parallel checks, exponential backoff, RELEASE tag fallback
- [ ] 34-02-PLAN.md — Extract shared service mappings to lib/service-map.sh, wire into health.sh and update.sh

---
*Roadmap created: 2026-03-17*
*Last updated: 2026-04-04 — Phase 34 planned: 2 plans in 1 wave*
