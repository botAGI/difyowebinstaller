...
**Plans:** 5 plans
- [ ] 02-01-PLAN.md — Peer detection (hw_detect_peer, _ensure_lldpd, _peer_ping_fallback) + phase_diagnostics wiring + unit test (PEER-01, PEER-02, PEER-03)
- [ ] 02-02-PLAN.md — lib/cluster_mode.sh (select/save/read) + _wizard_cluster_mode + AGMIND_MODE_OVERRIDE + --mode flag + 2 unit tests (CLUSTER-01, CLUSTER-02, PEER-04)
- [ ] 02-03-PLAN.md — templates/docker-compose.worker.yml + build_compose_profiles master-skip + lib/ssh_trust.sh + integration SSH test + image tag registry gate (COMPOSE-01, SSH-01)
