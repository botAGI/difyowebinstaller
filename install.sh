...
source "${INSTALLER_DIR}/lib/peer.sh"
...
# Derive phase count — single source of truth (lib/phases.sh PHASES array).
# Set AFTER TIMEOUT_START so any operator override of TIMEOUT_START flows into
# the PHASES array ${TIMEOUT_START:-300} expansion in lib/phases.sh.
...
BACKUPCRON
            ...
            # peer.sh fn — writes backup.conf REMOTE_BACKUP_* target (peer Spark via QSFP); no-op unless cluster mode=master
...
_smoke_peer_vllm_check() {
    ...
    if [[ -z "$peer_ip" ]]; then
        log_error "STRICT: cluster.json mode=master but peer_ip empty — peer_deploy inconsistency"
        return 1
    fi
    ...
    if ! curl -sSf --max-time 5 "http://${peer_ip}:8000/v1/models" >/dev/null 2>&1; then
        ...
        log_error "  Fix: rerun 'sudo bash install.sh' — peer_deploy idempotent"
        return 1
    fi
    local model
