...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/arch-aarch64%20%7C%20DGX%20Spark-blue" alt="Architecture">
  <img src="https://img.shields.io/badge/GPU-NVIDIA%20GB10-76b900" alt="GPU">
  <img src="https://img.shields.io/badge/containers-32%2B-orange" alt="Containers">
</p>

> **Поддерживаемая платформа:** `aarch64` / NVIDIA GB10 (DGX Spark и совместимые).
> Разработка под `x86_64` **приостановлена** начиная с v3.0.1 — весь фокус на ARM64
> unified memory архитектуре. Отдельные модули могут работать на x86, но не
> тестируются, и compose/image pinning заточен под arm64.
...
`ENABLE_OPENWEBUI`, `ENABLE_LITELLM`, `ENABLE_DOCLING`, `ENABLE_SEARXNG`, `ENABLE_DBGPT`, `ENABLE_CRAWL4AI`, `ENABLE_DIFY_PREMIUM`
...
- Работа идёт на `main` — без feature-веток и merge'ов
- PR собираются из `main` по команде
- Все Docker-образы привязаны к версиям через `versions.env`. Тег `:latest` запрещён
- Cluster deploy (master/worker) — единый процесс `install.sh` с авто-детектом peer через QSFP
