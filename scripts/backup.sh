...
if [[ -f "${INSTALL_DIR}/scripts/backup.conf" ]]; then
    while IFS='=' read -r key value; do
        ...
        case "$key" in
            BACKUP_RETENTION_COUNT|BACKUP_RETENTION_DAYS|ENABLE_S3_BACKUP|S3_REMOTE_NAME|S3_BUCKET|S3_PATH|ENABLE_BACKUP_ENCRYPTION|REMOTE_BACKUP_ENABLED|REMOTE_BACKUP_HOST|REMOTE_BACKUP_USER|REMOTE_BACKUP_KEY|REMOTE_BACKUP_PORT|REMOTE_BACKUP_PATH)
                declare "$key=$value" ;;
        esac
